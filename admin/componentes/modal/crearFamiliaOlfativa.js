...
export function crearFamiliaOlfativa(){
    ...
    let contenedor_h2 = crearElemento('h2', {}, `Crear FamiliaOlfativa`)
    ...
    contenedor_form.addEventListener('submit', (event) => {
        ...
        if(datos.get('familiaOlfativa').length < 3){
            ...
            fetch("../actions/familiaOlfativa/crearFamiliaOlfativa.php", {
                method: "POST",
                body: datos
            })
            .then(res => res.json())
            .then(data => {
                if(data == "existe"){
                    error("", "Ya existe una FamiliaOlfativa con ese nombre")
                }else if(data){
                    exito("", `Se creo la familiaOlfativa correctamente`, "?sec=fragancias")
                }else{
