...
export function crearProducto(marcas, generos, familiaOlfativas, concentraciones){
    let modal = document.querySelector('.modal') 
    modal.innerHTML = ''
    modal.classList.add('Activado')
    let contenedor = crearElemento('div', {
        class: "contenedorModal crearProducto"
    })
    let contenedor_h2 = crearElemento('h2', {}, `Crear un nuevo Producto`)
    let contenedor_form = crearElemento('form', {
        method: "POST",
        action: "",
        id: "crearProductoForm"
    })
    let contenedor_form_labelImagen = crearElemento('label', {
        for: "imagen"
    }, "Agregar una imagen")
    let contenedor_form_inputImagen= crearElemento('input', {
        type: "file",
        name: "imagen",
        id: "imagen",
        required: ""
    ...
    contenedor_form.append(
        contenedor_form_labelImagen, contenedor_form_inputImagen,
    ...
    contenedor_form.addEventListener('submit', (event) => {
        ...
        let validar = true
        datos.append('imagenLimpia', datos.get('imagen').name)
        ...
        if(validar){
            fetch("../actions/productos/crearProducto.php", {
                method: "POST",
                body: datos
            })
            .then(res => res.json())
            .then(data => {
                if(data == "existe"){
                    ...
                    exito("", `Se creo el producto correctamente`, "?sec=productos")
                }else{
    ...
    contenedor.append(contenedor_h2, contenedor_form)
